...
//!
//! The same format Aptos uses and a deliberately separate copy: the two chains
//! encode different structures, and a shared writer would invite one chain's
//! shape to be reached for while building the other's. What is shared is only
//! the rules - integers little-endian and fixed width, lengths and enum
//! variants as ULEB128 - and those are small enough to state twice.
//!
//! BCS is not self-describing, so a misplaced length byte does not produce a
//! malformed transaction. It produces a different one, signed correctly.
