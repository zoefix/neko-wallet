...
fn draw_settings(f: &mut Frame, area: Rect, app: &App, st: &SettingsState) {
    ...
    } else if matches!(st.row(), SettingRow::EtherscanKey) {
        lines.extend(hint_lines(t(Key::Settings_EtherscanKeyNote), inner.width));
        lines.extend(hint_lines(t(Key::Settings_EtherscanKeyNote2), inner.width));
