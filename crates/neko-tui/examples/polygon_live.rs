//! The Polygon path, end to end, against mainnet.
...
async fn main() {
    ...
    let client = neko_tui::chain::Client::for_chain(ChainId::Polygon, None, None);
    assert_eq!(
        client.chain(),
        ChainId::Polygon,
        "the client lost its chain"
    );
    let mine = ChainAddress::parse(ChainId::Polygon, &addr.to_string()).unwrap();
