//! The Base path, end to end, against mainnet.
...
async fn main() {
    ...
    let client = neko_tui::chain::Client::for_chain(ChainId::Base, None, None);
    assert_eq!(client.chain(), ChainId::Base, "the client lost its chain");
    let mine = ChainAddress::parse(ChainId::Base, &addr.to_string()).unwrap();
    ...
    let to =
        ChainAddress::parse(ChainId::Base, "0x742d35Cc6634C0532925a3b844Bc454e4438f44e").unwrap();
